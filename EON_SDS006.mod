...
HP{n in NN} = sum{f in FN}(sum{i in DN,j in DN,k in NN,m in NN:(i,j,k,m) in L}xp[n,i,j,k,m,f]); 
HB{n in NN} = sum{f in FN}(sum{i in DN,j in DN,k in NN,m in NN:(i,j,k,m) in L}xb[n,i,j,k,m,f]); 
