...
s.t. PRIMARY_INDEX_CONSISTENCE2{n in RN,f in FN}:
  (sum{i in DN,j in DN,k in NN,m in NN:(i,j,k,m) in L}(xp[n,i,j,k,m,f]))/(sum{h in FN}(sum{i in DN,j in DN,k in NN,m in NN:(i,j,k,m) in L}xp[n,i,j,k,m,h]))=
  p[n,f];
...
s.t. BACKUP_INDEX_CONSISTENCE2{n in RN,f in FN}:
  (sum{i in DN,j in DN,k in NN,m in NN:(i,j,k,m) in L}(xb[n,i,j,k,m,f]))/(sum{h in FN}(sum{i in DN,j in DN,k in NN,m in NN:(i,j,k,m) in L}xb[n,i,j,k,m,h])) =
  b[n,f];
end;
