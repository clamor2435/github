...
param HP{n in NN} := sum{f in FN}(sum{i in DN,j in DN,k in NN,m in NN:(i,j,k,m) in L}xp[n,i,j,k,m,f]); 
param HB{n in NN} := sum{f in FN}(sum{i in DN,j in DN,k in NN,m in NN:(i,j,k,m) in L}xb[n,i,j,k,m,f]); 
...
s.t. PRIMARY_INDEX_CONTINUE{n in RN,f in FN}:
	sum{i in DN, j in DN, k in NN, m in NN:(i,j,k,m) in L}xp[n,i,j,k,m,f] = HP;

s.t. BACKUP_INDEX_CONTINUE{n in RN,f in FN}:
	sum{i in DN, j in DN, k in NN, m in NN:(i,j,k,m) in L}xb[n,i,j,k,m,f] = HB;
