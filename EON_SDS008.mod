...
s.t. PRIMARY_INDEX_CONSISTENCE1{n in RN}:
  sum{f in FN}p[n,f] <= 1;

s.t. PRIMARY_INDEX_CONSISTENCE2{n in RN,f in FN}:
  sum{i in DN,j in DN,k in NN,m in NN:(i,j,k,m) in L}(xp[n,i,j,k,m,f]) =
  p[n,f] * sum{h in FN}(sum{i in DN,j in DN,k in NN,m in NN:(i,j,k,m) in L}xp[n,i,j,k,m,h]);

s.t. BACKUP_INDEX_CONSISTENCE1{n in RN}:
  sum{f in FN}b[n,f] <= 1;

s.t. BACKUP_INDEX_CONSISTENCE2{n in RN,f in FN}:
  sum{i in DN,j in DN,k in NN,m in NN:(i,j,k,m) in L}(xb[n,i,j,k,m,f]) =
  b[n,f] * sum{h in FN}(sum{i in DN,j in DN,k in NN,m in NN:(i,j,k,m) in L}xb[n,i,j,k,m,h]);
end;
