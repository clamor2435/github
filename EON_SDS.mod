...
s.t. LINK_DISJOINTNESS{n in RN,i in DN,j in DN,k in NN,m in NN:(i,j,k,m) in L}:
	p[n,i,j,k,m] + b[n,i,j,k,m] <= 1;

s.t. SAME_DOMAIN_SEQUENCE{n in RN,i in DN, j in DN:i != j}:
	sum{k in NN, m in NN:(i,j,k,m) in L}(p[n,i,j,k,m]) =
	sum{k in NN, m in NN:(i,j,k,m) in L}(b[n,i,j,k,m]);

s.t. PRIMARY_PASS_IS_GOOD{n in RN}:
	sum{i in DN,j in DN,k in NN,m in NN:(i,j,k,m) in L}(p[n,i,j,k,m]) <=
	sum{i in DN,j in DN,k in NN,m in NN:(i,j,k,m) in L}(b[n,i,j,k,m]);
/*
s.t. PRIMARY_INDEX_MAXIMUM{n in RN,i in DN,j in DN,k in NN,m in NN:(i,j,k,m) in L}:
	sum{f in FN}(yp[n,i,j,k,m,f]) <= p[n,i,j,k,m] * c[i,j,k,m];
s.t. PRIMARY_INDEX_MINIMUM{n in RN,i in DN,j in DN,k in NN,m in NN:(i,j,k,m) in L}:
	sum{f in FN}(yp[n,i,j,k,m,f]) >=p[n,i,j,k,m];
*/
end;
