...
s.t. PRIMARY_INDEX_CONSISTENCE2{n in RN,f in FN}:
  ...
  sum{h in FN}(sum{i in DN,j in DN,k in NN,m in NN:(i,j,k,m) in L}p[n,f] * xp[n,i,j,k,m,h]);
...
s.t. BACKUP_INDEX_CONSISTENCE2{n in RN,f in FN}:
  ...
  sum{h in FN}(sum{i in DN,j in DN,k in NN,m in NN:(i,j,k,m) in L}b[n,f] * xb[n,i,j,k,m,h]);
end;
