...
s.t. PRIMARY_PASS_HOP_COUNT{n in RN}:
  sum{f in FN}sum{i in DN,j in DN,k in NN,m in NN:(i,j,k,m) in L}(xp[n,i,j,k,m,f]) = p[n];
/*
s.t. BACKUP_PASS_HOP_COUNT{n in RN}:
  sum{f in FN}sum{i in DN,j in DN,k in NN,m in NN:(i,j,k,m) in L}(xb[n,i,j,k,m,f]) = b[n];

s.t. PRIMARY_PASS_IS_GOOD{n in RN}:
  p[n] <= b[n];
*/
s.t. PRIMARY_PASS_INDEX_CONTINUE{n in RN,f in FN}:
  sum{i in DN, j in DN, k in NN, m in NN:(i,j,k,m) in L}(xp[n,i,j,k,m,f]) = p[n];
/*
s.t. BACKUP_PASS_INDEX_CONTINUE{n in RN,f in FN}:
  sum{i in DN, j in DN, k in NN, m in NN:(i,j,k,m) in L}(xb[n,i,j,k,m,f]) = b[n];
*/
end;
