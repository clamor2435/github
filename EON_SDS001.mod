...
s.t. FIRST_INDEX_PRIMARY4{n in RN}:
  sum{i in DN,j in DN,k in NN,m in NN:(i,j,k,m) in L}(sum{f in FN}(xp[n,i,j,k,m,f])) = (1 / r[n])*sum{i in DN,j in DN,k in NN,m in NN:(i,j,k,m) in L}(sum{f in FN}(yp[n,i,j,k,m,f]));

s.t. FIRST_INDEX_BACKUP4{n in RN}:
  sum{i in DN,j in DN,k in NN,m in NN:(i,j,k,m) in L}(sum{f in FN}(xb[n,i,j,k,m,f])) = (1 / r[n])*sum{i in DN,j in DN,k in NN,m in NN:(i,j,k,m) in L}(sum{f in FN}(yb[n,i,j,k,m,f]));
